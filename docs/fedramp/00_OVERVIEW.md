...
> Last updated: 2026-05-30 (sprint 47)
...
| AC-2(11) Account Usage Conditions | 🔨 **Done** | `AccessHoursEnforcer.enforce()` called in `AuthService.login()` after credential validation; configurable UTC window (`app.access.allowed-hours.{enabled,start-hour,end-hour}`); overnight windows (e.g. 22–06) supported; `LOGIN_OUTSIDE_ALLOWED_HOURS` audit event with `utcHour`, `allowedStart`, `allowedEnd` details; `security.access.outside.hours` Micrometer counter; disabled by default for dev | — |
...
| IA-2(12) PIV Credentials | 🔨 **Done** | SP infrastructure ✅ · JIT provisioning ✅ · SAML success handler ✅ · SLO logout handler ✅ · login.gov sandbox registration pending deployment | [03_PIV_SAML](03_PIV_SAML.md) |
...
| IA-8(2) External Authenticators | 🔨 **Done** | Full SAML2 federation stack implemented (sprints 43–47) — pending login.gov sandbox registration | [03_PIV_SAML](03_PIV_SAML.md) |
...
| SC-23 Session Authenticity | 🔨 **Done** | Refresh token IP binding: `AuthService.refresh()` checks stored `ipAddress` against `request.getRemoteAddr()`; mismatch immediately revokes the token, emits `SESSION_IP_MISMATCH` audit event with `{ storedIp, requestIp }`, records `security.session.ip.mismatch` counter; null stored-IP (legacy rows) treated as satisfied (migration-safe); disabled by default (`SESSION_IP_BINDING_ENABLED=false`) — enable in prod for non-mobile workforces | — |
...
| IR-5 Incident Monitoring | 🔨 **Done** | `SecurityAnomalyService` runs every 5 min; queries `audit_events` for `MFA_LOCKOUT`, `SESSION_IP_MISMATCH`, `MFA_VERIFY_FAILED` counts within a rolling window; emits `SECURITY_ANOMALY_DETECTED` audit event with `{ eventType, count, threshold, windowSeconds }` when any threshold is breached; `security.anomaly.detected` counter; `SYSTEM_ACTOR` UUID identifies system-generated entries; thresholds configurable (`ANOMALY_MFA_LOCKOUT_THRESHOLD:3`, `ANOMALY_IP_MISMATCH_THRESHOLD:5`, `ANOMALY_MFA_FAILURE_THRESHOLD:10`) | — |
| IR-4 Incident Handling | 🔨 **Done** | `IncidentService` escalates `SECURITY_ANOMALY_DETECTED` bursts to incidents when count ≥ `anomalyThreshold` (default 3) in a 15-min window; emits `IR_INCIDENT_RAISED` audit event; Redis dedup key `incident:notified:ANOMALY_BURST` with configurable 1-hr cooldown prevents alert storms; `security.incident.raised` Micrometer counter; fully configurable via env vars (`INCIDENT_DETECTION_ENABLED`, `INCIDENT_ANOMALY_THRESHOLD`, `INCIDENT_WINDOW_SECONDS`, `INCIDENT_COOLDOWN_SECONDS`) | — |
| IR-6 Incident Reporting | 🔨 **Done** | `IncidentService.sendWebhook()` fires HTTP POST to `INCIDENT_WEBHOOK_URL` (PagerDuty/Slack/etc.) on each incident; fail-safe catch block logs warning and continues; blank URL skips HTTP call (log-only mode for dev) | — |
| SI-7 Software/Firmware Integrity | 🔨 **Done** | `FileIntegrityService` (`ApplicationRunner`) reads a `sha256sum`-compatible manifest at startup; computes SHA-256 of each listed file; emits `FILE_INTEGRITY_CHECK_PASSED` on success or one `FILE_INTEGRITY_VIOLATION` audit event + `security.integrity.violation` counter per affected file on failure; `app.integrity.fail-on-violation=true` triggers `IllegalStateException` for hard fail-fast in prod; manifest format supports `#` comments and blank lines; all config env-var controlled (`INTEGRITY_CHECK_ENABLED`, `INTEGRITY_MANIFEST_PATH`, `INTEGRITY_FAIL_ON_VIOLATION`) | — |
| SI-4 System Monitoring | ⚠️ Improved | 25 security-event Micrometer counters (auth, token, session, malware, MFA: `security.mfa.enrolled/disabled/verify.failed/backup.code.used/replay.attempt/lockout`, access hours: `security.access.outside.hours`, session integrity: `security.session.ip.mismatch`, anomaly: `security.anomaly.detected`, backup: `security.backup.verification.failed`, integrity: `security.integrity.violation`, incident: `security.incident.raised`); structured NDJSON audit log ready for SIEM tail ingestion; CloudWatch/ELK subscription still pending GovCloud | [05_AUDIT](05_AUDIT_SIEM.md) |
...
| Control | Status | Remaining Gap | Doc |
|---------|--------|---------------|-----|
| CP-9 Information System Backup | 🔨 **Done** | `BackupVerificationService` implements `HealthIndicator`; reads backup manifest file set by external backup script; reports `UP` (fresh), `DOWN` (missing or stale > 25 h), `UNKNOWN` (path not configured); `verifyBackup()` scheduled daily at 04:00 UTC; emits `BACKUP_VERIFICATION_PASSED/FAILED` audit event with health details; `security.backup.verification.failed` Micrometer counter on failure; all config env-var controlled (`BACKUP_VERIFICATION_ENABLED`, `BACKUP_MANIFEST_PATH`, `BACKUP_MAX_AGE_HOURS`, `BACKUP_VERIFICATION_CRON`); exposed at `/actuator/health` | — |

### Other Families (all ❌ Gap — documentation/process required)

| Family | Required Documents |
|--------|--------------------|
| CP — Contingency Planning | CP-9 backup verification ✅ Done · Contingency Plan, DR runbook (see [10_CONMON](10_CONMON.md)) |
...
| P0-3 | No PIV/CAC smart card support (IA-2(12)) | 🔨 **Done** | Full SAML2 SP stack ✅ (sprints 43–47) · **login.gov sandbox registration + cert exchange pending deployment** | [03_PIV_SAML](03_PIV_SAML.md) |
...
**Controls resolved or materially improved (sprint 47):** IA-2(12) + IA-8(2) (SAML2 Single Log-Out: `SamlLogoutHandler` implements `LogoutHandler` — `@ConditionalOnProperty(app.saml.enabled)` — delegates JWT revocation + cookie clearing to `authService.logout()` (AC-12/AC-10 parity with standard logout path), then emits `SAML_USER_LOGGED_OUT` audit event with `{provider, nameId}` when a `Saml2AuthenticatedPrincipal` is present and a `FederatedIdentity` row is found; null/non-SAML principal treated as no-op for SAML audit (JWT still revoked); wired into `Saml2SecurityConfig` via `logout().addLogoutHandler()`; 5 `SamlLogoutHandlerTest` tests — null auth, non-SAML principal, SAML principal with FI found/not found, always-revokes-JWT invariant; IA-2(12)/IA-8(2) code implementation complete (P0-3 resolved — login.gov sandbox registration pending deployment); BUILD SUCCESS, ≥ 0.95 branch coverage)  
**Controls resolved or materially improved (sprint 46):** IA-2(12) + IA-8(2) (SAML2 SP authentication wiring: `SamlAuthenticationSuccessHandler` — `@ConditionalOnProperty(app.saml.enabled)` — extracts `Saml2AuthenticatedPrincipal` fields (registrationId, nameId, attributes), calls `provisionOrLink()`, calls `authService.issueTokensForUser()` to set JWT cookies on the SAML response, then `sendRedirect()` to the Angular frontend (`SAML_FRONTEND_REDIRECT_URL`); `flattenAttributes()` collapses multi-valued SAML attribute lists to a flat map, skipping null/empty value lists; `Saml2SecurityConfig` — second `@Order(1)` `SecurityFilterChain` scoped to `/login/saml2/**`, `/logout/saml2/**`, `/saml2/**` using `saml2Login(successHandler)` + `saml2Logout(defaults)` so it captures only SAML2 paths and the JWT chain continues to handle everything else; `frontendRedirectUrl` added to `SamlProperties` and `application.yml`; 6 `SamlAuthenticationSuccessHandlerTest` tests covering SSO full path, attribute forwarding, and all `flattenAttributes` branches; BUILD SUCCESS, ≥ 0.95 branch coverage)  
**Controls resolved or materially improved (sprint 45):** IA-2(12) + IA-8(2) (SAML JIT user provisioning: `SamlUserProvisioningService.provisionOrLink()` implements three-path logic — (1) existing `FederatedIdentity` found: refresh `lastSeenAt` + attributes, emit `SAML_USER_LINKED` audit; (2) no FI but email matches active user: create FI link, emit `SAML_USER_LINKED` audit; (3) no match: JIT-provision new `User` + locked `Credential` (random inaccessible password) + default `EMPLOYEE` role assignment + `FederatedIdentity`, emit `SAML_USER_PROVISIONED` audit; blank/null email for new user throws `IllegalArgumentException`; `buildFullName()` prefers `name` attribute, falls back to `given_name`+`family_name`, defaults to `"Unknown"`; 12 `SamlUserProvisioningServiceTest` tests covering all three paths, deleted-user edge case, role assignment, null/blank email guards, and all `buildFullName` branches; BUILD SUCCESS, ≥ 0.95 branch coverage)  
**Controls resolved or materially improved (sprint 44):** IA-2(12) + IA-8(2) (federated identity data model: `federated_identities` table via V39 migration — `(provider, name_id)` unique constraint, JSONB `attributes` column, `last_seen_at` updated on every SSO login, FK to `users` with CASCADE DELETE, index on `user_id`; `FederatedIdentity` JPA entity with `@Builder.Default` timestamps and `@PreUpdate` lifecycle; `FederatedIdentityRepository` with `findByProviderAndNameId`, `findAllByUserId`, `existsByProviderAndNameId` derived queries; 6 `FederatedIdentityTest` tests — builder fields, `@Builder.Default` initialization, `@UniqueConstraint` reflection, column length annotation, `onUpdate()` lifecycle; BUILD SUCCESS, ≥ 0.95 branch coverage)  
**Controls resolved or materially improved (sprint 43):** IA-2(12) + IA-8(2) (SAML2 SP core infrastructure: `spring-security-saml2-service-provider` dep added; `SamlProperties` `@ConfigurationProperties(prefix="app.saml")` with SP fields + nested `Idp`; `Saml2RegistrationConfig` `@ConditionalOnProperty(app.saml.enabled)` builds `InMemoryRelyingPartyRegistrationRepository` with `RelyingPartyRegistration` for login.gov/PIV federation — `POST` ACS binding, `REDIRECT` SSO binding, `wantAuthnRequestsSigned=true`; `loadSigningCredentials()` + `loadVerificationCredentials()` load PKCS8 RSA key + X.509 cert from PEM files; all paths are blank-safe with warn-log; `app.saml.*` env-var config block in `application.yml`; Shibboleth repo added to `pom.xml` for OpenSAML resolution; 11 new `Saml2RegistrationConfigTest` tests using hardcoded PEM fixtures — registration fields, blank-path no-op, valid credential loading, invalid content exception paths, static parse helpers; BUILD SUCCESS, ≥ 0.95 branch coverage)  
**Controls resolved or materially improved (sprint 42):** IR-4 + IR-6 + SI-4 (incident response: `IncidentService` scheduled every 15 min; queries `SECURITY_ANOMALY_DETECTED` count in a rolling window; emits `IR_INCIDENT_RAISED` audit event + `security.incident.raised` counter (25th total) when burst threshold breached; Redis dedup suppresses repeat notifications within 1-hr cooldown; `sendWebhook()` fires HTTP POST to configurable endpoint for IR-6 reporting; blank URL defaults to log-only; 8 new `IncidentServiceTest` tests using `Mockito.spy()` for self-call interception + 2 `SecurityEventRecorderTest` tests; 819 total, BUILD SUCCESS, 95.49% branch coverage)  
**Controls resolved or materially improved (sprint 41):** SI-7 + SI-4 (file integrity verification: `FileIntegrityService` `ApplicationRunner` reads sha256sum-format manifest at startup; checks each file's SHA-256 against expected hash; emits `FILE_INTEGRITY_CHECK_PASSED` or per-file `FILE_INTEGRITY_VIOLATION` audit events; `security.integrity.violation` counter (24th total); `fail-on-violation` flag for prod fail-fast; 11 new `FileIntegrityServiceTest` tests using `@TempDir` + 1 `SecurityEventRecorderTest` test; 810 total, BUILD SUCCESS, 95.44% branch coverage)  
**Controls resolved or materially improved (sprint 40):** SC-28 (extend PII encryption to `users.full_name`: `@Convert(PiiEncryptionConverter)` added to `User.fullName`; column widened VARCHAR(120) → VARCHAR(300) via V38 migration; migration-safe ENC: prefix passthrough preserves existing plaintext rows; 1 new reflection test `user_fullName_isAnnotatedWithPiiEncryptionConverter`; 798 total, BUILD SUCCESS, 95.29% branch coverage)  
**Controls resolved or materially improved (sprint 39):** CP-9 + SI-4 (backup verification health check: `BackupVerificationService` implements `HealthIndicator`; checks backup manifest age against configurable `BACKUP_MAX_AGE_HOURS` (default 25); emits `BACKUP_VERIFICATION_PASSED/FAILED` audit events with health details; `security.backup.verification.failed` Micrometer counter (23rd total); scheduled daily at 04:00 UTC; 9 new `BackupVerificationServiceTest` tests using JUnit `@TempDir` for real file I/O; 1 new `SecurityEventRecorderTest` test; 797 total, BUILD SUCCESS, 95.29% branch coverage)  
**Controls resolved or materially improved (sprint 38):** IR-5 + SI-4 (security anomaly detection: `SecurityAnomalyService` scheduled every 5 min; `AuditEventRepository.countByActionSince()` JPQL count query; checks `MFA_LOCKOUT`/`SESSION_IP_MISMATCH`/`MFA_VERIFY_FAILED` against configurable thresholds in a rolling window; emits `SECURITY_ANOMALY_DETECTED` audit event + `security.anomaly.detected` counter on breach; all thresholds env-var configurable; 8 new `SecurityAnomalyServiceTest` tests + 1 `SecurityEventRecorderTest` test; 787 total, BUILD SUCCESS)  
**Controls resolved or materially improved (sprint 37):** SC-23 (session authenticity / refresh token IP binding: `AuthService.refresh()` validates client IP against stored IP; on mismatch revokes token + emits `SESSION_IP_MISMATCH` audit event + increments `security.session.ip.mismatch` counter; null stored-IP skips check for migration safety; `SESSION_IP_BINDING_ENABLED` env var, default false; `ipBindingEnabled` added to `AuthServiceTest.setUp()`; 4 new tests + 1 `SecurityEventRecorderTest` test; 778 total, BUILD SUCCESS)  
**Controls resolved or materially improved (sprint 36):** AU-2 (login/logout audit events: `USER_LOGGED_IN` emitted from `issueTokensForUser()` with `{ jti, ipAddress, userAgent }` details; `USER_LOGGED_OUT` emitted from `logout()` with `{ jti }` when valid access token present; completes FedRAMP AU-2 baseline requirement for logon/logoff event auditing; 3 new `AuthServiceTest` tests; 773 total, BUILD SUCCESS)  
